...
static usv_interfaces::msg::MmwaveTargetArray build_mmwave_target_array(
    const std_msgs::msg::Header& header,
    const std::vector<RadarObject>& objects)
{
    usv_interfaces::msg::MmwaveTargetArray msg;
    msg.header = header;
    msg.targets.reserve(objects.size());
    for (const auto& obj : objects) {
        usv_interfaces::msg::MmwaveTarget t;
        t.x = static_cast<double>(obj.xpos_vcs);
        t.y = static_cast<double>(obj.ypos_vcs);
        t.v_x = static_cast<double>(obj.xvel_abs);
        t.v_y = static_cast<double>(obj.yvel_abs);
        t.size_w = static_cast<double>(obj.box_width);
        t.size_l = static_cast<double>(obj.box_length);
        t.size_h = static_cast<double>(obj.box_heigh);
        t.objmotion_status = (obj.objmotion_status != 0U) ? static_cast<uint8_t>(1) : static_cast<uint8_t>(0);
        t.track_id = static_cast<uint32_t>(obj.track_id);
...
private:
    ...
    // 发布：系统目标列表 MmwaveTargetArray
    rclcpp::Publisher<usv_interfaces::msg::MmwaveTargetArray>::SharedPtr pub_target_array_;
    ...
    /**
     * 雷达报文内 PTP（秒/纳秒）→ rclcpp::Time。
     * 说明：切勿使用 rclcpp::Time(int64_t, int64_t) 误匹配到 (int32, uint32) 或把 uint64 秒强转
     * int64 后出现负秒，否则会抛出 “cannot store a negative time point in rclcpp::Time” 并终止进程。
     */
    rclcpp::Time time_from_radar_ptp(uint64_t ptp_sec, uint32_t ptp_nsec);

    /**
     * rclcpp::Time → builtin_interfaces::msg::Time（写入 Header）。
     * ROS 消息里 sec 为 int32：若 PTP 过大，隐式转换会得到负 sec，RViz 等节点会抛同样异常并崩溃。
     */
    builtin_interfaces::msg::Time stamp_msg_from_ros_time(const rclcpp::Time& t);
...
public:
    ...
    explicit mmw_radar(
        ...
        std::string raw_pointcloud_topicName = "/mmw_radar/raw_pointcloud_topic",
        std::string raw_object_topicName = "/mmw_radar/raw_objectList_topic",
        std::string target_array_topicName = "/perception/radar/mmw/objects",
        ...
        pub_pointPtr_PointCloud2_ =
            this->create_publisher<sensor_msgs::msg::PointCloud2>(raw_pointcloud_topicName, 10);
        pub_object_pc2_ =
            this->create_publisher<sensor_msgs::msg::PointCloud2>(raw_object_topicName, 10);
        pub_object_marker_ =
            this->create_publisher<visualization_msgs::msg::MarkerArray>(object_marker_topicName, 10);
        pub_target_array_ = this->create_publisher<usv_interfaces::msg::MmwaveTargetArray>(
            target_array_topicName, 10);
        ...
        while (rclcpp::ok()) {
            ...
            switch(messageType) {
                ...
                case 0xAA80:
                    ...
                    radar_ptp_time = time_from_radar_ptp(ptp_s, ptp_ns);
                ...
                case 0xAA81:
                    if(0x01 == findPointCloudHeader) {
                        ...
                        if(pointcloud_num > 0 && points.size() >= static_cast<size_t>(pointcloud_num)) {   
                            ...
                            // 初始化PointCloud2消息
                            cloud_msg.header.stamp = stamp_msg_from_ros_time(radar_ptp_time);
                ...
                if(findObjectHeader == 0x01) {
                    ...
                    if(saveObjectNum == object_num ) {
                        ...
                        publish_object_outputs(radar_ptp_time, objects);
                    }
                } 
                break;
    ...
    void publish_object_outputs(const rclcpp::Time& stamp, const std::vector<RadarObject>& objects)
    {
        if (objects.empty()) {
            return;
        }
        ...
        object_pc2.header.stamp = stamp_msg_from_ros_time(stamp);
        object_pc2.header.frame_id = frame_id_;
        object_pc2.width = static_cast<uint32_t>(objects.size());
                        object_pc2.height = 1;
        ...
        pub_target_array_->publish(build_mmwave_target_array(object_pc2.header, objects));
    }
};

builtin_interfaces::msg::Time mmw_radar::stamp_msg_from_ros_time(const rclcpp::Time& t)
{
    constexpr int64_t k_ns_per_sec = 1000000000LL;
    constexpr int64_t k_max_header_ns =
        static_cast<int64_t>(std::numeric_limits<int32_t>::max()) * k_ns_per_sec + (k_ns_per_sec - 1LL);

    auto ns_to_header_in_range = [](int64_t ns_total) -> builtin_interfaces::msg::Time {
        constexpr int64_t k_ns = 1000000000LL;
        builtin_interfaces::msg::Time out;
        out.sec = static_cast<int32_t>(ns_total / k_ns);
        out.nanosec = static_cast<uint32_t>(ns_total % k_ns);
        return out;
    };

    auto fallback_clock_stamp = [this, k_max_header_ns, ns_to_header_in_range]() -> builtin_interfaces::msg::Time {
        try {
            const int64_t ns = this->get_clock()->now().nanoseconds();
            if (ns >= 0LL && ns <= k_max_header_ns) {
                return ns_to_header_in_range(ns);
            }
        } catch (...) {
        }
    ...
    int64_t ns_total = 0;
    try {
        ns_total = t.nanoseconds();
    } catch (const std::runtime_error&) {
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "无法读取 header 对应 rclcpp::Time 的纳秒，改用节点时钟");
        return fallback_clock_stamp();
    }

    if (ns_total < 0LL || ns_total > k_max_header_ns) {
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "header.stamp 超出 ROS msg Time(sec:int32) 可表示范围或为负，改用节点时钟（避免 RViz 崩溃）");
        return fallback_clock_stamp();
    }

    return ns_to_header_in_range(ns_total);
}

rclcpp::Time mmw_radar::time_from_radar_ptp(uint64_t ptp_sec, uint32_t ptp_nsec)
{
    constexpr int64_t k_ns_per_sec = 1000000000LL;
    constexpr int64_t k_max_header_ns =
        static_cast<int64_t>(std::numeric_limits<int32_t>::max()) * k_ns_per_sec + (k_ns_per_sec - 1LL);

    while (ptp_nsec >= 1000000000U) {
        ptp_nsec -= 1000000000U;
        ++ptp_sec;
    }
    // int64 总纳秒不可溢出；并避免 uint64 秒直接 static_cast<int64_t> 产生“负秒”
    const uint64_t max_sec_for_ns =
        static_cast<uint64_t>(std::numeric_limits<int64_t>::max()) /
        static_cast<uint64_t>(k_ns_per_sec);
    if (ptp_sec > max_sec_for_ns) {
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "雷达 PTP 秒字段异常过大 (sec=%llu)，使用节点时钟作为 header.stamp",
            static_cast<unsigned long long>(ptp_sec));
        return this->get_clock()->now();
    }
    const int64_t total_ns =
        static_cast<int64_t>(ptp_sec) * k_ns_per_sec + static_cast<int64_t>(ptp_nsec);
    if (total_ns < 0) {
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "雷达 PTP 换算得到负纳秒，使用节点时钟作为 header.stamp");
        return this->get_clock()->now();
    }
    // 超出 int32 sec 时写入 Header 会溢出成负秒，RViz 解析必崩
    if (total_ns > k_max_header_ns) {
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 5000,
            "雷达 PTP 超出 ROS Header Time(sec:int32) 上限，使用节点时钟作为 header.stamp");
        return this->get_clock()->now();
    }
    try {
        return rclcpp::Time(total_ns, RCL_SYSTEM_TIME);
    } catch (const std::runtime_error & e) {
        RCLCPP_WARN(
            this->get_logger(),
            "雷达 PTP 无法构成 rclcpp::Time (%s)，使用节点时钟作为 header.stamp", e.what());
        return this->get_clock()->now();
    }
}
...
int main(int argc, char **argv) {
    ...
    const std::string raw_pointcloud_topic = bootstrap_node->declare_parameter<std::string>(
        "raw_pointcloud_topic", "/mmw_radar/raw_pointcloud_topic");
    const std::string raw_object_topic = bootstrap_node->declare_parameter<std::string>(
        "raw_object_topic", "/mmw_radar/raw_objectList_topic");
    const std::string target_array_topic = bootstrap_node->declare_parameter<std::string>(
        "target_array_topic", "/perception/radar/mmw/objects");
    const std::string object_marker_topic = bootstrap_node->declare_parameter<std::string>(
        "object_marker_topic", "/mmw_radar/object_marker");
