...
- `reference_frame`：与 Gazebo 世界/桥接 TF 一致的坐标系（常用 `map`）；`message_frame_id`：真值消息头与 `percision_sim` 一致的坐标系（默认 `base_link`），二者不同时通过 TF 将 `x,y,v` 与 Marker 变换到 `message_frame_id`。
